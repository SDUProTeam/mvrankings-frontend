...
const useStyles = makeStyles((theme) => ({
  ...
  subBarContent: {
    flexGrow: 1,
    padding: theme.spacing(3),
    maxWidth: `calc(100% - ${window.innerWidth - document.body.clientWidth}px)`,
    backgroundColor: "white",
  },
}));

const navLabels = ["电影展厅", "表格展示", "", "个性化推荐"];
const navIcons = [<AppIcon />, <ArtTrackIcon />, undefined, <PaletteIcon />];
...
export function SubTopBar(props) {
  ...
  return (
    <div className={classes.colRoot}>
      ...
      <div className={classes.below}>{props.below}</div>
